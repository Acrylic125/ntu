...
class Trie:
    ...
    def find_words_with_prefix(self, prefix): #question 2
        ...
        results = []
        def _find_words_wirth_prefix(node, prefix, acc):
            cur = node.first_child
            while cur != None:
                if prefix == "":
                    if cur.is_end_of_word:
                        results.append(f"{acc}{cur.char}")
                    _find_words_wirth_prefix(cur, prefix[1:], f"{acc}{cur.char}")
                elif prefix[0] == cur.char:
                    # if len(prefix) == 1:
                    #     results.append(f"{acc}{cur.char}")
                    _find_words_wirth_prefix(cur, prefix[1:], f"{acc}{cur.char}")
                cur = cur.next_sibling
        _find_words_wirth_prefix(self.root, prefix, "")
        return results
    ...
    def count_words(self, node): #question 1
        ...
        while cur != None:
            if cur.is_end_of_word:
                count = count + 1
            count = count + self.count_words(cur.first_child)
            cur = cur.next_sibling
        return count
